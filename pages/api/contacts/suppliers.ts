...
async function handler(req: ApiRequest, res: NextApiResponse) {
    ...
    const collection = db.collection<Client>('suppliers')

    if (req.method === 'GET') {
        const suppliers = await collection.find().toArray()
        return res.status(200).json(suppliers.map(supplier => {
            return sanitizeClientForResponse(supplier as Record<string, unknown>)
        }))
    }
    ...
    if (req.method === 'POST') {
        ...
        if (existing) {
            throw new ConflictError('Supplier with this uid already exists', { uid })
        }
        ...
        await collection.insertOne(encryptStringsDeep(newSupplier))
        return res.status(201).json(sanitizeClientForResponse(newSupplier as unknown as Record<string, unknown>))
    }
...
export default withApi(handler, {
    requiredPermissionsByMethod: {
        GET: ['viewSuppliers'],
        POST: ['viewSuppliers', 'manageSuppliers'],
        PUT: ['viewSuppliers', 'manageSuppliers'],
        PATCH: ['viewSuppliers', 'manageSuppliers'],
        DELETE: ['viewSuppliers', 'manageSuppliers'],
    },
